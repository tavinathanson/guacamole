...
object Reference {
  ...
  def makeBasesAtLoci(referenceLines: RDD[(Reference.Locus, Array[Byte])],
    ...
    referenceLines.flatMap({
      case ((contigName, readStart), bytes) =>
        val contigStart: Long = broadcastContigStart.value(contigName)
        val start = contigStart + readStart
        val positions = Array.tabulate(bytes.length) { i => start + i }
        for (i <- 0 until positions.length) yield (positions(i), bytes(i))
    }).cache()
  }
