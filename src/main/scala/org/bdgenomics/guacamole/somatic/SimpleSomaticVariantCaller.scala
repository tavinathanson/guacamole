...
object SimpleSomaticVariantCaller extends Command {
  ...
  def buildPileups(reads: RDD[SimpleRead],
    ...
    var baseReadsAtPos: RDD[(Long, BaseRead)] = reads.flatMap(expandBaseReads(_, broadcastContigStart, minBaseQuality))
  ...
  def callVariants(tumorReads: RDD[SimpleRead],
    ...
    def readKey(read: SimpleRead) = {
      ...
      val offset = read.start
      broadcastContigStart.value(contig) + offset
    }
    ...
    val tumorPileups = buildPileups(tumorReadsPartitioned, broadcastContigStart, minBaseQuality, minTumorCoverage, Some(partitioner))

    Common.progress("-- Tumor pileups: %s with %d partitions".format(
      tumorPileups.getClass, tumorPileups.partitions.size))

    val normalPileups = buildPileups(normalReadsPartitioned, broadcastContigStart, minBaseQuality, minNormalCoverage, Some(partitioner))
    ...
    genotypes.keyBy({
      genotype: ADAMGenotype =>
        val contigName = genotype.getVariant.getContig.getContigName.toString
        val contigStart = broadcastIndex.value.contigStart(contigName)
        contigStart + genotype.getVariant.getPosition
    }).sortByKey().values
  }
  ...
  def callVariantsFromArgs(sc: SparkContext, args: Arguments): RDD[ADAMGenotype] = {
    ...
    val reference = Reference.load(referencePath, sc, numSplits = Some(100))
